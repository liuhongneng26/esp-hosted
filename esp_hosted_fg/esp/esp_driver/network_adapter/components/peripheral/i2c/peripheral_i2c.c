...
static int _do_i2cdetect_cmd(i2c_master_bus_handle_t tool_bus_handle)
{
    uint8_t address;
...
int i2cdetect(i2c_port_t i2c_port)
{
    if (i2c_port < 0 || i2c_port >= I2C_NUM_MAX)
    {
        ESP_LOGE(TAG, "not find i2c-%d", i2c_port);
        return -1;
    }

    if (g_i2c_master_bus_handle[i2c_port] == NULL)
    {
        ESP_LOGE(TAG, "not init i2c-%d", i2c_port);
        return -1;
    }

    return _do_i2cdetect_cmd(g_i2c_master_bus_handle[i2c_port]);
}
...
esp_err_t i2c_master_init(i2c_port_t i2c_port, gpio_num_t sda, gpio_num_t scl)
{
    ...
    _do_i2cdetect_cmd(g_i2c_master_bus_handle[i2c_port]);
...
int i2c_master_add(i2c_port_t i2c_port, i2c_master_dev_handle_t *dev_handle, uint16_t device_address, uint32_t hz)
{
    ...
    esp_err_t ret = i2c_master_probe(g_i2c_master_bus_handle[i2c_port], device_address, I2C_TOOL_TIMEOUT_VALUE_MS);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "not find i2c %X", device_address);
        return -1;
    }
    ...
    return i2c_master_bus_add_device(g_i2c_master_bus_handle[i2c_port], &dev_cfg, dev_handle);
}

i2c_master_bus_handle_t i2c_master_get_hand(i2c_port_t i2c_port)
{
    if (i2c_port >= I2C_NUM_MAX)
        return NULL;

    return g_i2c_master_bus_handle[i2c_port];
}
